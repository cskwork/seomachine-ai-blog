...
기존에는 AI 도구마다 고유한 통합 방식을 사용했습니다. MCP는 이를 하나의 표준으로 통합하며, "AI를 위한 USB-C"에 비유됩니다.

### 해결하는 문제

- 도구마다 별도의 통합 코드를 작성해야 하는 문제 (N x M 문제)
- 데이터 소스 접근 방식의 파편화
- 보안과 권한 관리의 비일관성
- 컨텍스트 공유의 어려움
...
LLM이 읽을 수 있는 데이터를 제공합니다. 파일, 데이터베이스 레코드, API 응답 등을 포함합니다.

**2. Tools (도구)**

LLM이 실행할 수 있는 작업을 정의합니다. 데이터베이스 쿼리, API 호출, 파일 조작 등이 있습니다.
