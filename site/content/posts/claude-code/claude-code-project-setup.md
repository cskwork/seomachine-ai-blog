...
date: 2026-03-18
draft: false
tags: ["Claude Code", "프로젝트 설정", "CLAUDE.md", "claudeignore"]
categories: ["Claude Code"]
description: "Claude Code 프로젝트 초기 설정 완벽 가이드. /init, CLAUDE.md, .claudeignore 설정법과 프로젝트 규모별 최적 구성을 다룹니다."
...
`.gitignore`가 Git 추적에서 파일을 제외하듯, `.claudeignore`는 Claude Code의 컨텍스트에서 파일을 제외한다. 문법도 `.gitignore`와 동일하다.

제외해야 할 파일의 원칙은 두 가지다. 첫째, Claude Code가 읽어도 도움이 되지 않는 파일이다. 의존성 디렉토리, 빌드 산출물, 바이너리 파일 등이 여기에 해당한다. 둘째, 보안상 노출되면 안 되는 파일이다. 환경 변수 파일, 인증서, 자격증명 파일이 이에 해당한다.

반면 제외하면 안 되는 파일도 있다. 설정 파일(webpack.config.js, tsconfig.json 등), 테스트 파일, 문서 파일은 Claude Code가 프로젝트를 이해하는 데 핵심적인 역할을 한다. 이 파일들을 제외하면 Claude Code의 제안 품질이 크게 떨어진다.

다음은 일반적인 웹 프로젝트에서 사용할 수 있는 `.claudeignore` 예시다.
...
마지막 주석이 중요하다. `test-results/`처럼 테스트 실행 결과는 제외하더라도, `tests/` 디렉토리 자체는 절대 제외하면 안 된다.
...
`.claudeignore` 파일은 `node_modules/`와 `dist/` 정도만 포함하거나, 아예 생성하지 않아도 된다. Claude Code가 읽어야 할 파일이 많지 않기 때문에 컨텍스트 오염 위험이 낮다.
...
이 규모에서는 `.claudeignore` 설정이 중요하다. 빌드 산출물과 의존성 디렉토리를 제외하지 않으면 Claude Code가 관련 없는 파일을 읽느라 컨텍스트를 낭비한다.
...
`.claudeignore`도 이에 맞게 구성한다. 현재 작업과 무관한 패키지는 제외하고, 공유 라이브러리는 반드시 포함하는 방식이 효과적이다.
...
초기 설정에 투자하는 시간은 반드시 회수된다. CLAUDE.md를 잘 작성해두면 매번 같은 컨텍스트를 반복해서 설명하지 않아도 되고, `.claudeignore`를 올바르게 설정하면 Claude Code가 핵심 파일에 집중할 수 있다. 프로젝트가 성장함에 따라 이 설정 파일들도 함께 발전시켜 나가는 것이 장기적으로 가장 효율적인 접근 방식이다.
