...
Claude Code를 `--mcp-debug` 플래그와 함께 실행하면 MCP 통신 내용을 로그로 볼 수 있다.

```bash
claude --mcp-debug
```

서버가 도구 목록에 아예 나타나지 않는 경우는 대부분 `settings.json` 경로 문제다. 파일을 불러온 위치(`~/.claude/settings.json`인지 `.claude/settings.json`인지)와 JSON 문법 오류 여부를 먼저 확인한다.
