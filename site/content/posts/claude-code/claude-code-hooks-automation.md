...
`matcher`는 어떤 도구에 반응할지 지정한다. 각 항목 안에 `hooks` 배열로 실행할 명령을 정의하며, `type`은 `"command"`로 설정한다. Hook 명령은 도구의 입력 정보를 표준 입력(stdin)으로 JSON 형태로 전달받는다.
...
이 로그를 주기적으로 검토하거나 SIEM 시스템으로 전송하면 Claude의 Bash 실행 이력을 추적할 수 있다. 더 상세한 로깅이 필요하면 Hook 명령에서 stdin으로 전달되는 JSON을 파싱하여 실행된 명령의 세부 내용을 기록할 수 있다.
...
Hook 명령은 도구 입력을 stdin으로 전달받는다. `exit 1`을 반환하면 Claude Code는 해당 도구 실행을 차단하고, 표준 출력의 메시지를 Claude에게 전달한다. Claude는 이 피드백을 받아 대안을 제시하게 된다.
